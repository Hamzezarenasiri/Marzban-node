...
SINGBOX_ENABLED = config("SINGBOX_ENABLED", cast=bool, default=True)
SINGBOX_EXECUTABLE_PATH = config("SINGBOX_EXECUTABLE_PATH", default="/usr/local/bin/sing-box")
SINGBOX_WORKING_DIR = config("SINGBOX_WORKING_DIR", default="/var/lib/marzban-node")
...
SINGBOX_INBOUNDS = config("SINGBOX_INBOUNDS", cast=lambda v: [x.strip() for x in v.split(',')] if v else [], default="")
